...
1. In order to migrate Parts Unlimited web site, we have to create an App Service Plan. The Azure App Service Migration Assistant will take care of all the requirements needed. Select **Use existing (1)** and select the lab resource group as your deployment target. App Service requires a globally unique Site Name, we suggest using a pattern that matches **partsunlimited-web-<inject key="DeploymentID" enableCopy="false"/>** **(2)**. Select the region where your resources have been currently deployed to. (you can find this by opening the your allocated **Resource Group** in the Azure Portal. Click **Migrate** to start the migration process.
